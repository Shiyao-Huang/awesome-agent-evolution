...
\begin{enumerate}
    ...
    \begin{align}
        ...
        \Delta\mathcalI, \Delta\mathcal{O} &= \text{LLM}(\mathcal{P}_{\text{update}}(\mathcal{I}, \mathcal{O}, \nabla_{\text{lang}}))
    \end{align}
\end{enumerate}
...
Table~\ref{tab:benchmark_compilation} consolidates the quantitative results from the methods surveyed in this section, enabling direct comparison across benchmarks and model scales.

\begin{table}[t]
\centering
\small
\caption{Benchmark results compilation for self-improvement methods. All results are as reported in the original papers.}
...
The most striking finding is \textbf{size-equivalence}: self-improvement enables small models to match or exceed large models (STaR's 540M matching 175B; Reflexion's GPT-3.5 surpassing GPT-4). This has profound implications for the economics of AI deployment---self-improvement can substitute for scale.

\subsection{Cross-Cutting Observations}
\label{sec:cross-cutting}

\begin{enumerate}
    \item \textbf{The generate-filter-train pattern is universal.} Every training-time method follows: generate candidates, filter for quality, train on filtered data. Methods differ in filtering mechanism and training objective, but the structure is identical.
    \item \textbf{Inference-time and training-time methods are complementary.} Use inference-time methods for immediate gains; periodically distill improved outputs into parameters through training-time methods.
    \item \textbf{The evaluation bottleneck persists.} Quality of self-improvement is bounded by quality of evaluation. Developing better self-evaluation mechanisms is the most critical open problem.
    \item \textbf{Convergence is fast (2--3 iterations).} Both SPIN and ReST-EM converge in $\sim$3 iterations. The most effective strategy is periodic self-training interspersed with other improvements.
    \item \textbf{Self-improvement is most effective in structured domains.} The strongest results come from code (passes/fails tests), math (correct answers), and reasoning (ground truth). Open-ended tasks show weaker improvements.
    \item \textbf{The Echo Trap reveals a fundamental challenge.} RAGEN's discovery that agents can learn to ``fake'' improvement by echoing outputs demonstrates that self-evaluation must be carefully monitored in self-evolving systems.
\end{enumerate}
