# 从零构建能自我优化的AI Agent：Reflection和Reflexion机制对比详解与实现-腾讯云开发者社区-腾讯云
...
**Source**: https://cloud.tencent.com/developer/article/2588362
...
![](https://developer.qcloudimg.com/http-save/yehe-7220647/b798d4ba0ff56f308ac897bd32d24e99.jpg)
...
我们这里按需逐步引入，保持学习流程的清晰度。首先用`.env`文件设置API集成的环境变量：
...
`ChatPromptTemplate`用来组织提示，包含两个部分：
...
管道操作符充当桥梁：将`post_creation_prompt`的输出直接传递给LLM，使其能根据用户指令生成结构化帖子。
...
What's your experience with AI dev tools? Share your metrics below! 👇  
 ...
 #AIforDevelopers #DevTools #SoftwareDevelopment #CodingTools #DeveloperLife #TechInnovation
```
...
定义`ContentState`来组织工作流。结合`Annotated`和`add_messages`，确保消息在整个过程中正确累积。
...
![](https://developer.qcloudimg.com/http-save/yehe-7220647/642ed5b821566f4d4d42a3486329493f.jpg)

#### 测试运行

最后用简单示例测试自动化的`linkedin_workflow`：
...
![](https://developer.qcloudimg.com/http-save/yehe-7220647/bdfa7b99b76bbbb2957134b48269132d.jpg)
...
![](https://developer.qcloudimg.com/http-save/yehe-7220647/53c7206a9ee94152e1c865522702452a.jpg)
...
由于Reflexion需要外部知识，首先定义从网络获取信息的工具。这里用`TavilySearchResults`——Tavily搜索API的封装，让Agent能进行网络搜索并收集支撑证据。
...
用Pydantic的`BaseModel`定义两个数据类：

`Reflection`捕捉自我批评，要求Agent指出哪些信息缺失和哪些多余。`GenerateResponse`构造最终输出，确保Agent提供主要回答、包含基于`Reflection`类的反思，并提供`research_queries`列表。
...
将`GenerateResponse`模型作为工具绑定，这会强制LLM严格按照定义的结构输出。
...
调用`initial_response_chain`后，会得到包含初始答案、自我批评和生成的搜索查询的结构化输出。用简单查询测试初始响应器：
...
为强制输出结构需要引入Pydantic schema `ImproveResponse`，它类继承自`GenerateResponse`并增加`sources`字段，确保每个改进的答案都附带可验证的参考来源。
...
![](https://developer.qcloudimg.com/http-save/yehe-7220647/fc1f3b00c7a9ded05473adea3af942e0.jpg)
...
虽然这些方法可能需要更多LLM调用（因而耗费更多时间和成本），但它们显著提升了产出高质量、可靠结果的可能性。更重要的是，通过在记��中存储改进轨迹或用于微调，能帮助模型避免未来重复相同错误。
...
原始发表：2025-09-29，如有侵权请联系 [cloudcommunity@tencent.com](mailto:cloudcommunity@tencent.com) 删除

[工具](https://cloud.tencent.com/developer/tag/17276)

[工作流](https://cloud.tencent.com/developer/tag/17285)

[优化](https://cloud.tencent.com/developer/tag/17554)

[agent](https://cloud.tencent.com/developer/tag/11736)

[reflection](https://cloud.tencent.com/developer/tag/15657)
