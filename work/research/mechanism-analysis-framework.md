...
language: zh-CN
source_corpus:
  survey: survey/ch1-ch8
  paper_drafts: paper-drafts/ch1-ch8
output_type: mechanism_framework
evidence_level: derived-from-survey+paper-drafts
---
...
**Reflexion 的 verbal RL**：Actor 执行到 Evaluator 返回反馈到 Self-Reflection 生成语言教训到 Episodic Memory 保存到 下一轮读取。核心创新是把稀疏的 0/1 reward 转化为语义梯度。证据：Ch3.3, Ch4.4。

**ACE 的上下文工程**：把上下文视为可演化 playbook，使用结构化增量更新避免上下文坍缩。证据：Ch3.3, Ch3.6。

### 2.4 局限性

1. 反思幻觉：LLM 可能错误归因失败原因 [review-2303.11366]
2. 上下文膨胀：长任务中观察记录挤压可用上下文窗口 [P015, P068]
3. 观察遗漏：工具调用、状态转换可能未被完整捕获 [P013]
4. 观察偏差：自我观察带有模型自身认知盲点 [P010, P054]
...
**AlphaEvolve 的自动 evaluator**：候选程序放入可执行 evaluator，用性能指标作为 fitness function。覆盖矩阵乘法、数据中心调度等。核心：只有当问题可以被程序化验证时，进化闭环才可靠。证据：Ch4.3。

**Self-Rewarding LM 的评价循环**：同一模型既生成又评价，通过 Iterative DPO 训练。关键风险：长度从 M1 的 1092 tokens 涨到 M3 的 2552 tokens，部分提升可能来自 length gaming。证据：Ch4.5。

### 3.5 局限性

1. Goodhart 定律：当评估指标成为优化目标，指标就会失真 [Ch5.3.2, P022]
2. 评价器退化：长度偏见、位置偏见、score distribution collapse [review-2407.19594]
3. 评估覆盖不足：单一 benchmark 无法覆盖真实生产需求 [Ch5.3.1]
4. 评估-修改隔离难题：Agent 不应能直接修改自己的评估器 [Ch8.3]
...
基于 Ch2.4：`z_{t+1}, A_{t+1} = S(A_t union {U_k(z_t, x_t, y_t)}_{k=1}^K; V, C, D)`
...
**DGM 代码自修改闭环**：Archive 中 agent 变体到 采样父代到 修改自身 Python 代码到 Sandbox 执行到 Benchmark 评估到 Archive 更新。SWE-bench 20.0% 到 50.0%。证据：Ch4.1。

**Voyager 技能库进化**：自动课程到 技能检索到 GPT-4 生成代码到 环境执行到 成功技能写入 skill library。3.3x unique items，15.3x tech tree 解锁速度。证据：Ch4.4。

### 4.6 局限性

1. 改进 Plateau：多数系统初期有收益，很快进入平台期 [Ch7.2]
2. 归因困难：多模块同时变化时无法确定改进来源 [Ch3.6]
3. 灾难性遗忘：学习新能力时可能丢失旧能力 [Ch8.4]
4. 成本爆炸：搜索成本可能远超收益 [Ch7.5]
...
**DGM 开放式 Archive**：不只保留最强个体，而是维护多样 archive。不同分支探索不同局部最优，某些当前不强的变体可能成为未来 stepping stone。证据：Ch4.1。

**EvoMAC 多agent网络**：把协作网络中的节点和边视为可更新单元，用文本反馈调整协作结构。证据：Ch3.4。

### 5.5 局限性

1. 共识幻觉：多个模型互相肯定错误答案 [Ch8.2]
2. 成本二次增长：辩论成本随agent数和轮数近似二次增长 [Ch7.5]
3. 异质性不足：多数多agent系统只是同一模型的多次调用 [Ch8.2]
4. 协调失败：角色分工和handoff不完善导致任务丢失 [P013]
...
**AlphaEvolve 56年突破**：4x4 复数矩阵乘法使用 48 次标量乘法，相比 Strassen 49 次实现改进。不是人类设计的，而是 LLM+进化搜索多代积累涌现的。证据：Ch4.3。

**DGM 自发现能力**：系统自动发现更好的代码编辑工具、长上下文管理策略和 peer-review 机制。证据：Ch4.1。

**RAGEN Echo Trap**：多轮 RL agent 在自身生成的状态-思考-动作模式中重复错误。负向涌现。证据：Ch3.1。

### 6.5 局限性

1. 可预测性差：涌现行为难以事前预测 [Ch8.3]
2. 风险评估滞后：负向涌现常在长时间运行后才显现 [P086]
3. 安全边界模糊：能力增长和风险增长可能同步 [Ch8.3]
4. 评估不充分：benchmark 难以捕获涌现行为质量 [Ch5.3]
