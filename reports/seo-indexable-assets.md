# SEO Indexable Assets Audit

Generated: 2026-06-05T16:31:13.721Z

Canonical host: https://agent-evolution.com

| Metric | Count |
|---|---:|
| Assets checked | 564 |
| Passed | 564 |
| Failed | 0 |
| Sitemap URLs | 981 |

Global status: **PASS**

## Scope

This audit verifies generated HTML SEO assets for `site/src/content/blog/*.mdx`, `site/src/content/research/*.mdx`, `site/public/reports/projects/*.md`, `site/public/reports/research/projects/*.md`, `site/public/reports/papers/cross-domain/*.md`, and `reports/survey-publication/[0-9][0-9]-*.md`: canonical URL, sitemap inclusion, meta description, Open Graph metadata, JSON-LD article type, and absence of `noindex`. Machine index files such as `site/public/reports/projects/INDEX.md` are excluded as non-article indexes.

**Boundary:** this is a generated-site asset audit, not a live Google crawl report. `PASS` means the checked source files have generated HTML assets with indexable metadata in the local build; it does not prove Google has crawled or indexed the pages, and it does not clear external custom-domain HTTPS certificate blockers.

## Failures

- None.

## Global Failures

- None.
