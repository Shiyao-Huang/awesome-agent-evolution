...
\begin{equation}
    ...
    = \mathcal{U}\bigl(\theta_t, m_t, x, y_t, \mathcal{F}(x,y_t,\tau_t)\bigr),
    \label{eq:generic-self-improvement}
\end{equation}
where $x$ is the task input, $y_t$ is the current output, $\tau_t$ is the execution or reasoning trajectory, $m_t$ is non-parametric state such as memory or retrieved demonstrations, $\theta_t$ denotes model parameters, $\mathcal{F}$ is a feedback source, and $\mathcal{U}$ is an update rule. In inference-time self-improvement, $\theta_{t+1}=\theta_t$ and the update occurs in $y$ or $m$: the model rewrites an answer, adds reflection text to context, or changes the next action plan. In training-time self-improvement, $\theta$ changes: the feedback is converted into new supervised data, preference pairs, filtered trajectories, rewards, or policy gradients. In agent-level self-evolution, $\theta$ may remain fixed while the executable system around the model changes, for example through prompts, tools, workflows, or code patches. The distinction is not merely taxonomic; it determines the cost, safety profile, reproducibility, and failure modes of the method.
...
We organize the chapter into four parts. Section~\ref{subsec:inference-time-self-improvement} analyzes inference-time self-improvement, emphasizing Self-Refine, Reflexion, and Self-Debug. Section~\ref{subsec:training-time-self-improvement} analyzes training-time methods, including SPIN, STaR, ReST-EM, Constitutional AI, RISE, RAGEN, and related verifier-based code agents. Section~\ref{subsec:comparison-table} provides a compact method comparison. Section~\ref{subsec:benchmark-data} consolidates benchmark numbers from the research corpus and the primary papers, with special attention to HumanEval, GSM8K, BBH, MMLU, and SWE-Bench. Throughout, we treat self-improvement as a loop design problem: What state is updated? What feedback is trusted? What prevents drift? What is the unit of selection? How is improvement measured?
...
\begin{align}
    a_e &\sim \pi_a(\cdot \mid x, \mathcal{H}_{e-1}), \\
    r_e &= \pi_v(x,a_e,\tau_e), \\
    f_e &\sim \pi_r(\cdot \mid x,a_e,\tau_e,r_e), \\
    \mathcal{H}_e &= \operatorname{UpdateMemory}(\mathcal{H}_{e-1}, f_e), \\
    a_{e+1} &\sim \pi_a(\cdot \mid x, \mathcal{H}_e),
    \label{eq:reflexion-loop}
\end{align}
where $e$ indexes episodes, $a_e$ is the action or answer, $\tau_e$ is the trajectory, $r_e$ is a scalar or binary evaluation, $f_e$ is a verbal reflection, and $\mathcal{H}_e$ is the memory buffer after episode $e$ (following the $h_t$ notation of Chapter~\ref{sec:taxonomy-five-loops}). If one distinguishes successful and failed episodes, the update can be expressed as
\begin{equation}
    \mathcal{H}_e = \mathcal{H}_{e-1} \cup
    \begin{cases}
    \{\pi_r(x,a_e,\tau_e,r_e)\}, & r_e < \kappa,\\
...
\label{alg:reflexion}
\begin{algorithmic}[1]
\Require Task $x$, actor $\pi_a$, evaluator $\pi_v$, reflector $\pi_r$, memory budget $B$, episode budget $E$
\State $\mathcal{H}_0 \gets \varnothing$
\For{$e=1$ to $E$}
    \State $a_e,\tau_e \gets \pi_a(x,\mathcal{H}_{e-1})$
    \State $r_e \gets \pi_v(x,a_e,\tau_e)$
    \If{$\textsc{Success}(r_e)$}
        \State \Return $a_e$
    \Else
        \State $f_e \gets \pi_r(x,a_e,\tau_e,r_e)$
        \State $\mathcal{H}_e \gets \textsc{CompressAndAppend}(\mathcal{H}_{e-1}, f_e, B)$
    \EndIf
\EndFor
\State \Return best action according to $\pi_v$
\end{algorithmic}
\end{algorithm}

The benchmark signature of Reflexion is unusually strong for a non-parametric method. The local research corpus reports $91.0\%$ pass@1 on HumanEval \cite{humaneval2021} using Reflexion with GPT-3.5, compared with $80.1\%$ for GPT-4 zero-shot and $67.0\%$ for fine-tuned Codex \cite{chen2021codex}. It also reports $97\%$ success on ALFWorld compared with a $75\%$ base, and improvements on MBPP, APPS, and WebShop. These numbers should be interpreted carefully. Reflexion uses repeated attempts and evaluation signals, so it is not the same compute regime as one-shot generation. Nevertheless, the results demonstrate that language memory can substitute for parameter updates in tasks where failure signals are available and where future attempts can condition on previous mistakes.
...
\begin{equation}
    (c_{t+1},\mathcal{H}_{t+1})=
    \Bigl(R_\theta(x,c_t,o_t,d_t),\ \operatorname{Append}\bigl(\mathcal{H}_t, \pi_r(x,c_t,o_t)\bigr)\Bigr).
\end{equation}
...
STaR \cite{zelikman2022star}, the Self-Taught Reasoner of Zelikman et al., is a rationale bootstrapping method. Its starting observation is that chain-of-thought rationales improve reasoning, but human-written rationales are expensive. If a model can generate rationales, and if the final answer can be checked, then correct self-generated rationales can be used as training data. Over iterations, the model becomes better at producing rationales that lead to correct answers. The loop is simple and powerful: generate rationales, keep those that yield correct answers, optionally rationalize failures by conditioning on the correct answer, fine-tune, and repeat.
...
Reflexion \cite{shinn2023reflexion} & Cross-episode inference memory & No parameter update & Evaluator reward plus verbal reflection memory & Code generation, ALFWorld, WebShop, interactive agents & HumanEval pass@1 $91.0\%$; GPT-4 zero-shot reference $80.1\%$; Codex fine-tuned $67.0\%$; ALFWorld $97\%$ vs $75\%$ base \\
Self-Debug \cite{singh2023selfdebug} / SelfEvolve \cite{jiang2023selfevolve} & Inference-time execution repair & No for repair; optional training & Compiler, interpreter, tests, tracebacks & Code generation, data-science programming, translation between languages & SelfEvolve DS-1000 $49.4\rightarrow57.2$; HumanEval pass@1 $74.39\rightarrow85.98$; TransCoder accuracy $88.3\rightarrow90.4$ \\
SPIN \cite{singh2024beyond} & Iterative self-play fine-tuning & Yes & Human SFT response vs previous model response & Chat alignment, general LLM benchmarks & Zephyr-7B GSM8K $26.76\rightarrow38.97$; average leaderboard $58.14\rightarrow63.16$; SPIN+DPO average $64.05$ \\
...
ReST-EM \cite{singh2024beyond} & Grow--Improve offline RL / EM & Yes & Reward model filtering or weighting & Machine translation, preference alignment, best-of-$N$ distillation & ReST Improve steps raise reward scores across IWSLT/WMT/Web; second Grow adds $5.3$ points on IWSLT and $0.8$ on Web Domain \\
...
RISE \cite{qu2024rise} & Multi-turn RL for introspection & Yes & Final-answer reward over reasoning trajectory & Math reasoning, multi-step QA & GSM8K Llama2-7B roughly $14\%\rightarrow24\%$; Mistral-7B roughly $38\%\rightarrow46\%$ \\
RAGEN \cite{wang2025ragen} & Trajectory-level agent RL & Yes & Step and trajectory rewards; echo filtering & Tool agents, multi-turn decision tasks & StarPO/StarPO-S improves multi-turn agent training and identifies Echo Trap / reasoning collapse \\
Absolute Zero \cite{zhao2025absolute} & Self-play task proposal and solving & Yes & Code executor verification & Code, math, logic with verifiable rewards & AZR-Coder-7B reported SOTA at 7B level on coding average; zero external training data \\
ReVeal \cite{jin2025reveal} & Multi-turn code RL with self-verification & Yes & Generated tests, tool execution, turn-level reward & Code agents, LiveCodeBench & $20+$ inference turns despite training on $3$ turns; self-verification optimized explicitly \\
\end{longtable}}
...
Benchmark claims in self-improvement papers require careful normalization. A loop that uses five attempts and a verifier should not be compared naively with a one-shot model. A method that trains on answer-labeled datasets should not be described as zero-data. A method that improves an average score may still regress on individual tasks. This section consolidates exact numbers available in the project research corpus and primary papers for HumanEval, GSM8K, BBH, MMLU, and SWE-Bench. The purpose is not to crown a single winner, but to show which feedback loops have evidence on which evaluation regimes.
...
\midrule
HumanEval \cite{humaneval2021} & Reflexion \cite{shinn2023reflexion} (GPT-3.5) & GPT-4 zero-shot $80.1\%$; Codex fine-tuned $67.0\%$ & $91.0\%$ pass@1 & Inference-time verbal memory with evaluator \\
...
GSM8K \cite{gsm8k2021} & SPIN \cite{singh2024beyond} on Zephyr-7B-SFT-full & $26.76$ & $38.97$ at iteration 3 & Self-play fine-tuning; MMLU slightly decreases \\
GSM8K \cite{gsm8k2021} & RISE \cite{qu2024rise} & Llama2-7B $\sim14\%$; Mistral-7B $\sim38\%$ & $\sim24\%$; $\sim46\%$ & Multi-turn RL introspection \\
GSM8K \cite{gsm8k2021} & STaR \cite{zelikman2022star} & $5.8\%$ answer-only & $10.7\%$ & Rationale bootstrapping on early models \\
BBH subset & SPIN \cite{singh2024beyond} causal judgment & $56.15$ & $59.36$ at iteration 2 & Few-shot CoT BBH subset \\
...
MMLU & SPIN \cite{singh2024beyond} on Zephyr-7B-SFT-full & $60.92$ & $59.99$ at iteration 3; $59.68$ after SPIN+DPO & Shows non-monotonic per-benchmark behavior \\
...
Reinforcement learning is appropriate when the system must learn a policy over multi-turn actions, tool calls, or revision decisions. RISE \cite{qu2024rise}, RAGEN \cite{wang2025ragen}, and ReVeal \cite{jin2025reveal} are examples. RL is powerful because it can optimize delayed rewards and non-differentiable outcomes, but it is expensive and unstable. It requires careful reward design, KL control, exploration management, and degeneracy diagnostics. RL should not be the first tool for a simple formatting or one-shot reasoning problem. It becomes necessary when the behavior to be learned is sequential and cannot be reduced to independent supervised examples.
...
Self-improvement methods form the engine of AI self-evolution. Self-Refine \cite{madaan2023selfrefine} shows that a single model can improve outputs through generate--critique--refine loops. Reflexion \cite{shinn2023reflexion} shows that natural-language reflections can serve as non-parametric memory and approximate verbal reinforcement learning. Self-Debug \cite{singh2023selfdebug} shows that execution feedback can ground repair. SPIN \cite{singh2024beyond} shows that a model can use its own previous responses as self-play negatives to improve a supervised seed. STaR \cite{zelikman2022star} shows that rationales can be bootstrapped from answer correctness. ReST-EM \cite{singh2024beyond} shows how generated candidates and reward filtering create a growing-batch training loop. Constitutional AI \cite{bai2022constitutional} shows how a compact human constitution can be expanded into critiques, revisions, and AI feedback for alignment. RISE \cite{qu2024rise}, RAGEN \cite{wang2025ragen}, Absolute Zero \cite{zhao2025absolute}, and ReVeal \cite{jin2025reveal} show the frontier: multi-turn RL, trajectory-level credit assignment, self-generated curricula, and explicit self-verification.
