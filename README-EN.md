...
|---|---|---|
| [algorithmicsuperintelligence/openevolve](https://github.com/algorithmicsuperintelligence/openevolve) | Repaired the raw capture from an `unknown` timestamp to a 2026-06-03 public GitHub-page observation, then propagated the new star/fork snapshot into classification, site data, and the public report. | It is one of the strongest evolutionary coding-agent infrastructure samples in the corpus, so leaving a bogus local `6 stars` value would distort both ranking and narrative. |
| [EverMind-AI/EverOS](https://github.com/EverMind-AI/EverOS) | Refreshed the raw snapshot, memory-theme classification, site data, and model card with the public-page 6.8k / 669 star-fork snapshot and the visible MCP/skills surfaces. | It links long-term memory, skills, MCP, and self-evolution benchmark suites in one public repository, making it a core memory-substrate anchor. |
| [microsoft/SkillOpt](https://github.com/microsoft/SkillOpt) | Rewrote the skill-optimizer metadata from the 2026-06-03 public GitHub page, lifting the snapshot from 1.1k / 100 to 4.5k / 452 while keeping the API-blocked freshness caveat explicit. | It shows that the trainable state can be the skill itself, which is strong evidence for skill-layer self-evolution rather than model finetuning. |
| [RyanAlberts/best-of-Agent-Harnesses](https://github.com/RyanAlberts/best-of-Agent-Harnesses) | Refreshed the harness-landscape raw/site/report surfaces and wrote the README's own "stars captured on 2026-05-31" methodology note back into the evidence chain. | It is a useful public comparison surface for the 2026 harness ecosystem and a reader-facing index for navigating harness choices. |
| [pinchbench/skill](https://github.com/pinchbench/skill) | Updated the benchmark raw/site/report snapshot to 2026-06-03, corrected fork / issue counts, and preserved the transcript-retention plus leaderboard-upload method signals. | It provides real tasks, an external judge, and retained transcripts, which is exactly the benchmark surface needed to decide whether agent improvement is believable. |
