...
|---|---|---|
| [anthropics/skills](https://github.com/anthropics/skills) | Rewrote the raw capture and public report from a cached GitHub result observed on 2026-06-04, moving the official skills repository to `142k / 16.8k` while making it explicit that the underlying crawl was `last week`, not a live API fetch. | It is the upstream standards anchor for the skills ecosystem; stale size and install-path evidence would skew the entire skill-layer narrative. |
| [benchflow-ai/skillsbench](https://github.com/benchflow-ai/skillsbench) | Refreshed the benchmark raw, classification row, site card, and public report from `1.2k / 302 / 360 commits` to `1.3k / 305 / 367 commits`, and restored the `tasks_excluded` plus lockfile reproducibility signals. | It is the clearest public benchmark surface for asking whether skills actually improve agent behavior on real tasks. |
| [browser-use/browser-harness](https://github.com/browser-use/browser-harness) | Refreshed the browser-harness raw capture and public report with cached GitHub evidence for `13.9k / 1.3k / 391 commits / 13 issues / 91 PRs`. | It connects browser automation, editable helpers, domain skills, and runtime self-healing, making it a high-value harness-evolution sample. |
| [cxxz/awesome-agent-memory](https://github.com/cxxz/awesome-agent-memory) | Refreshed the raw capture, memory report, and site card from a cached GitHub result observed on 2026-06-04, restoring `MIT`, `11 / 3`, issue / PR counts, and a fuller memory-taxonomy description. | It is not a runtime but a memory knowledge entry point, so refreshing it helps keep the memory-substrate layer broader than a few implementation repos. |
| [ai-boost/awesome-harness-engineering](https://github.com/ai-boost/awesome-harness-engineering) | Rechecked the raw capture and public report, making it explicit that this run could only re-confirm the harness taxonomy through a cached GitHub result while `1.1k / 100` remains the best local count evidence. | It is the thematic entry point for harness engineering; the important part is to preserve the evidence boundary instead of overstating freshness. |
