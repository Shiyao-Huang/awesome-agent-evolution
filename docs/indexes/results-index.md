# Results / 结果输出 Index

> Generated: 2026-06-03

## Rule

可交付、可发布、可下载、可部署的输出归这里；生成物要说明来源和刷新命令。

## Coverage

| Path | Status | Files | Directories | Skipped | Size |
|---|---|---:|---:|---:|---:|
| `reports` | present | 14 | 2 | 0 | 98 KB |
| `output` | present | 8 | 1 | 0 | 684 KB |
| `site/public/reports` | present | 511 | 6 | 0 | 3.6 MB |
| `site/dist` | present | 981 | 433 | 0 | 37 MB |
| `paper-drafts/main.pdf` | present | 1 | 0 | 0 | 1.6 MB |
| `docs/publishing-readiness-check.md` | present | 1 | 0 | 0 | 3.5 KB |

## Representative Files

### `reports`

- `reports/academic-propagation-analysis.md`
- `reports/cross-validation-report.md`
- `reports/current-value-ranking-reanalysis.md`
- `reports/evolve-agi-index-trend.json`
- `reports/project-discovery-leads.md`
- `reports/propagation-chain-evidence.md`
- `reports/star-analysis-report.md`
- `reports/survey-publication/01-field-map.md`
- `reports/survey-publication/02-true-vs-pseudo-evolution.md`
- `reports/survey-publication/03-evaluation-bottleneck.md`

### `output`

- `output/raw-data-timestamp-validation-report.json`
- `output/raw-github-timestamp-index-ZH.md`
- `output/raw-github-timestamp-index.json`
- `output/raw-github-timestamp-index.md`
- `output/raw-papers-timestamp-index.json`
- `output/raw-papers-timestamp-index.md`
- `output/social-media-curated-ZH.md`
- `output/social-media-curated.md`

### `site/public/reports`

- `site/public/reports/papers/cross-domain/README.md`
- `site/public/reports/papers/cross-domain/relationship-map.mmd`
- `site/public/reports/papers/cross-domain/sources.md`
- `site/public/reports/projects/01-opro-llm-as-optimizer.md`
- `site/public/reports/projects/02-openelm-evolution-large-models.md`
- `site/public/reports/projects/03-adas-automated-design-agentic-systems.md`
- `site/public/reports/projects/04-funsearch-mathematical-discoveries.md`
- `site/public/reports/projects/05-automl-agent-multi-agent.md`
- `site/public/reports/projects/06-coml-mlcopilot.md`
- `site/public/reports/projects/07-metagpt-multi-agent-framework.md`

### `site/dist`

- `site/dist/CNAME`
- `site/dist/_astro/BaseLayout.Dnh_NTE-.css`
- `site/dist/_astro/ValueLshGraph3D.astro_astro_type_script_index_0_lang.DEPfkaIg.js`
- `site/dist/_astro/client.BpEju3qh.js`
- `site/dist/_astro/index.BIbOj7I9.css`
- `site/dist/_astro/index.BPKjW9fM.css`
- `site/dist/_astro/index.BfFUp1ss.css`
- `site/dist/_astro/index.CBk-yZE3.css`
- `site/dist/_astro/index.CmLyJJUK.css`
- `site/dist/_astro/index.Djbw_tfD.css`

### `paper-drafts/main.pdf`

- `paper-drafts/main.pdf`

### `docs/publishing-readiness-check.md`

- `docs/publishing-readiness-check.md`
