# Root Document Map

> Generated: 2026-05-26

Root only keeps entry points, legal/release files, and compatibility files still referenced by scripts or research notes. New management documents belong in `docs/`.

| File | Class |
|---|---|
| `AGENTS.md` | ops |
| `CLAUDE.md` | ops |
| `CLOUD.md` | ops |
| `CODE_OF_CONDUCT.md` | release/legal |
| `CONTENT_INDEX.md` | ops |
| `CONTRIBUTING.md` | release/legal |
| `DELIVERY_SUMMARY.md` | ops |
| `LICENSE-CODE` | release/legal |
| `LICENSE-CONTENT` | release/legal |
| `NOTICE` | release/legal |
| `PAPER_OUTLINE.md` | work compatibility |
| `README-EN.md` | ops |
| `README-ZH.md` | ops |
| `README.md` | ops |
| `SECURITY.md` | release/security |
| `awesome-social-media-resources-ZH.md` | processed compatibility |
| `awesome-social-media-resources.md` | processed compatibility |
| `cross-validation-report.md` | processed compatibility |
| `github-agent-evolution-repos-ZH.md` | processed compatibility |
| `github-agent-evolution-repos.md` | processed compatibility |
| `mom-test-findings-ZH.md` | raw compatibility |
| `mom-test-findings-chinese.md` | raw compatibility |
| `mom-test-findings-hn-ZH.md` | raw compatibility |
| `mom-test-findings-hn.md` | raw compatibility |
| `mom-test-findings-reddit-ZH.md` | raw compatibility |
| `mom-test-findings-reddit.md` | raw compatibility |
| `mom-test-findings-x-ZH.md` | raw compatibility |
| `mom-test-findings-x.md` | raw compatibility |
| `mom-test-findings.md` | raw compatibility |
| `raw-data-timestamp-standard.md` | raw compatibility |
| `raw-data-timestamp-validation-report.json` | raw compatibility |
| `raw-github-index.md` | raw compatibility |
| `social-media-raw-data-ZH.md` | raw compatibility |
| `social-media-raw-data.md` | raw compatibility |
