...
[Skip to content](https://openai.github.io/openai-agents-python/mcp/#model-context-protocol-mcp)
...
For a full pause/resume flow, see [Human-in-the-loop](https://openai.github.io/openai-agents-python/human_in_the_loop/) and `examples/mcp/get_all_mcp_tools_example/main.py`.
...
If the MCP server implements the HTTP with SSE transport, instantiate [MCPServerSse](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerSse). Apart from the transport, the API is identical to the Streamable HTTP server.
...
For MCP servers that run as local subprocesses, use [MCPServerStdio](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerStdio). The SDK spawns the process, keeps the pipes open, and closes them automatically when the context manager exits. This option is helpful for quick proofs of concept or when the server only exposes a command line entry point.
...
When you have multiple MCP servers, use `MCPServerManager` to connect them up front and expose the connected subset to your agents. See the [MCPServerManager API reference](https://openai.github.io/openai-agents-python/ref/mcp/manager/) for constructor options and reconnect behavior.
...
Use [create_static_tool_filter](https://openai.github.io/openai-agents-python/ref/mcp/util/#agents.mcp.util.create_static_tool_filter) to configure simple allow/block lists:
...
For more elaborate logic pass a callable that receives a [ToolFilterContext](https://openai.github.io/openai-agents-python/ref/mcp/util/#agents.mcp.util.ToolFilterContext). The callable can be synchronous or asynchronous and returns `True` when the tool should be exposed.
