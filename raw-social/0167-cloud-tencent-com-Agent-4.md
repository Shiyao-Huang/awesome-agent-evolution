# Agent设计模式——第 4 章：反思-腾讯云开发者社区-腾讯云
...
**Source**: https://cloud.tencent.com/developer/article/2581250

---

[https://cloud.tencent.com/developer/user/6468007](https://cloud.tencent.com/developer/user/6468007)

[xindoo](https://cloud.tencent.com/developer/user/6468007)
...
![作者头像](https://developer.qcloudimg.com/http-save/10011/4b8502c2cf48084865bfbf3c6a9f6393.jpg)

xindoo

发布于 2025-10-27 09:35:44
...
[XINDOO的专栏](https://cloud.tencent.com/developer/column/90754)
XINDOO的专栏
...
1. 规划与策略：  评估提议计划并识别潜在缺陷或改进点

- **用例：** 目标达成行动规划 Agent
- **反思：** 生成计划，模拟执行或根据约束评估可行性，基于评估修订计划
- **优势：** 制定更有效和现实的计划

1. 对话 Agent：  审查对话历史轮次以保持上下文、纠正误解或提升响应质量
...
代码首先设置环境，加载 API 密钥，并使用低温度初始化强大的语言模型（如 GPT-4o）以获得专注输出。核心任务由提示定义，要求创建计算数字阶乘的 Python 函数，包括文档字符串、边界情况（0 的阶乘）和负输入错误处理的特定要求。run*reflection*loop 函数协调迭代优化过程。在循环中，第一次迭代时语言模型根据任务提示生成初始代码，后续迭代中则基于前一步的评审优化代码。单独的"反思者"角色（同样由语言模型扮演但使用不同系统提示）充当高级软件工程师，根据原始任务要求评审生成的代码。此评审以问题项目符号列表或短语 'CODE*IS*PERFECT'（如无问题）形式提供。循环持续至评审指示代码完美或达到最大迭代次数。对话历史被维护并在每一步传递给语言模型，为生成/优化和反思阶段提供上下文。最后，脚本在循环结束后打印最终生成的代码版本
...
此代码演示了在 Google ADK 中使用顺序 Agent 管道生成和审查文本。它定义了两个 LlmAgent 实例：generator 和 reviewer。generator Agent 设计用于创建关于给定主题的初始草稿段落，被指示撰写简短信息丰富的文章，并将其输出保存至状态键 draft*text。reviewer Agent 作为生成器产出文本的事实核查员，被指示从 draft*text 读取文本并验证其事实准确性。评审者的输出是包含两个键的结构化字典：status 和 reasoning。status 指示文本为"ACCURATE"或"INACCURATE"，reasoning 则提供状态解释。此字典保存至状态键 review*output。创建名为 review*pipeline 的 SequentialAgent 来管理两个 Agent 的执行顺序，确保生成器先运行，然后是评审者。整体执行流程为：生成器产出文本并保存至状态，随后评审者从状态读取文本，执行事实核查，并将其发现（状态和推理）保存回状态。此管道允许使用独立 Agent 进行结构化内容创建和审查过程。**注意：** 对于感兴趣者，还提供了利用 ADK 的 LoopAgent 的替代实现。
...
![](https://developer.qcloudimg.com/http-save/yehe-6468007/a852fd2af5f73507c0aad17b67e44a09.png)
...
![](https://developer.qcloudimg.com/http-save/yehe-6468007/6b2368f897fc21a6e3582e19cb7b367a.png)
...
1. Training Language Models to Self-Correct via Reinforcement Learning, [https://arxiv.org/abs/2409.12917](https://cloud.tencent.com/developer/tools/blog-entry?target=https%3A%2F%2Farxiv.org%2Fabs%2F2409.12917&objectId=2581250&objectType=1&contentType=undefined)
2. LangChain Expression Language (LCEL) Documentation: [https://python.langchain.com/docs/introduction/](https://cloud.tencent.com/developer/tools/blog-entry?target=https%3A%2F%2Fpython.langchain.com%2Fdocs%2Fintroduction%2F&objectId=2581250&objectType=1&contentType=undefined)
3. LangGraph Documentation:[https://www.langchain.com/langgraph](https://cloud.tencent.com/developer/tools/blog-entry?target=https%3A%2F%2Fwww.langchain.com%2Flanggraph&objectId=2581250&objectType=1&contentType=undefined)
4. Google Agent Developer Kit (ADK) Documentation (Multi-Agent Systems): [https://google.github.io/adk-docs/agents/multi-agents/](https://cloud.tencent.com/developer/tools/blog-entry?target=https%3A%2F%2Fgoogle.github.io%2Fadk-docs%2Fagents%2Fmulti-agents%2F&objectId=2581250&objectType=1&contentType=undefined)

> 本文来源于开源项目[Agentic Design Patterns中文翻译](https://cloud.tencent.com/developer/tools/blog-entry?target=https%3A%2F%2Fgithub.com%2Fxindoo%2Fagentic-design-patterns&objectId=2581250&objectType=1&contentType=undefined)

本文参与 [腾讯云自媒体同步曝光计划](https://cloud.tencent.com/developer/support-plan)，分享自作者个人站点/博客。

 原始发表：2025-10-04 ，如有侵权请联系 [cloudcommunity@tencent.com](mailto:cloudcommunity@tencent.com) 删除

[agent](https://cloud.tencent.com/developer/tag/11736)

[函数](https://cloud.tencent.com/developer/tag/17290)

[模型](https://cloud.tencent.com/developer/tag/17381)

[设计模式](https://cloud.tencent.com/developer/tag/17424)
