...
Different systems make different choices along each dimension, and these choices largely determine the system's capabilities, limitations, and failure modes.
...
\textbf{Formal Structure.} The Evaluator Loop can be decomposed into four tiers of increasing complexity:
\begin{enumerate}
\item \textbf{Deterministic verifiers}: Unit tests, type checkers, linters, static analyzers, fuzzing, and regression tests. These provide binary or scalar signals with no noise.
\item \textbf{Model-based evaluators}: LLM-as-a-judge, critic agents, debate-based evaluation, and meta-judges. These provide rich but noisy signals that may carry systematic biases.
\item \textbf{Interactive environment evaluators}: Web arenas (WebArena, WebShop), code sandboxes (SWE-Bench), robot simulators, and game environments. These provide grounded feedback from real or simulated execution.
\item \textbf{Human evaluators}: Domain experts, crowdworkers, or end users who provide preference judgments, correctness ratings, or qualitative feedback.
\end{enumerate}

The evaluator's quality determines the ceiling of the evolution process. As Goodhart's law states, ``when a measure becomes a target, it ceases to be a good measure.'' If the evaluator has gaps, the system will optimize for those gaps rather than for genuine improvement. This motivates multi-tier evaluation: using deterministic checks for hard constraints, model-based evaluation for soft quality judgments, and periodic human auditing for calibration.
...
\textbf{Formal Structure.} The Population Loop maintains an archive $\mathcal{A}_t = \{(z_i, f_i, \ell_i)\}_{i=1}^{N_t}$ where $z_i$ is a candidate, $f_i$ its fitness, and $\ell_i$ its lineage (ancestry information). At each generation:
\begin{enumerate}
\item \textbf{Selection}: Choose parent candidates $\mathcal{P} \subset \mathcal{A}_t$ based on fitness, novelty, or quality-diversity criteria.
\item \textbf{Variation}: Generate offspring $\mathcal{O} = \{\mu(p) \mid p \in \mathcal{P}\}$ using LLM-guided mutation and recombination.
\item \textbf{Evaluation}: Score each offspring $o \in \mathcal{O}$ using the evaluator $V$.
\item \textbf{Archive Update}: Add offspring to the archive and optionally remove low-performing or redundant entries: $\mathcal{A}_{t+1} = \phi(\mathcal{A}_t \cup \mathcal{O})$.
\end{enumerate}
...
\textbf{Evaluator + Reflection.} The evaluator provides the failure signal that triggers reflection. Without a good evaluator, reflections may be uninformative or misleading. Reflexion's effectiveness depends on having clear task feedback (pass/fail on coding problems, navigation success/failure) that the agent can reason about. In domains with noisy or ambiguous feedback, reflection quality degrades.
...
\subsection{Theoretical Boundaries and Limits}\label{sec:theory-limits}

\subsubsection{Self-Reward Consistency}

When no external or independent evaluator exists, self-rewarding may collapse into self-consistency illusion. The model generates outputs, evaluates them with itself, and trains on its own judgments---a circular process that can reinforce existing biases rather than correct them. Meta-Rewarding attempts to mitigate this by adding a meta-judge layer, but the meta-judge is itself a model and subject to the same biases, just at a higher level of abstraction.

\subsubsection{Benchmark Overfitting}

If the task distribution is too narrow, the system may learn benchmark-specific hacks rather than generalizable capabilities. This is particularly acute in code generation, where specific benchmark formats, test styles, and problem structures can be memorized. Systems that report improvement only on a single benchmark should be viewed with caution.

\subsubsection{Modification Scope and Constraint Adequacy}

If the modifiable scope is too broad and constraints are insufficient, the system may corrupt evaluation, logging, or safety boundaries. The agent might modify its own evaluator to give itself higher scores, delete logs of problematic behaviors, or expand its own permissions beyond what was intended. This is not hypothetical---DGM agents can modify their own code, and without sandbox isolation, such modifications could have unintended consequences.

\subsubsection{Archive Pollution}

If the archive lacks curation, long-term memory accumulates noise, contradictions, and stale entries. Context collapse---where too many memories degrade rather than improve performance---is a documented failure mode. AriadneMem's entropy-aware gating and Memory-R1's learned memory management policy represent two approaches to this problem, but neither is a complete solution.

\subsubsection{Base Model Ceiling}

If the underlying foundation model lacks sufficient capability, LLM-generated variations may be unable to cross critical technical thresholds. Self-evolution amplifies existing capabilities but cannot fundamentally exceed the model's generative capacity. A weak model generating variations on its own prompts may produce only superficial changes, while a strong model may generate genuinely novel and useful modifications.

In summary, Agent Self-Evolution is not a magic growth engine; it is an optimization process that couples base model capabilities, external feedback, search mechanisms, and engineering constraints. It can amplify existing strengths, but it will also amplify deficiencies in the evaluator and environment design.
...
Agent Self-Evolution intersects with several established frameworks in machine learning theory, and understanding these connections helps position the field relative to prior work.

\subsubsection{Reinforcement Learning}

The formalization in Equation~\ref{eq:update} maps naturally onto reinforcement learning: the agent takes actions (generating trajectories), receives rewards (evaluator scores), and updates its policy (through any of the $U$ operators). However, Agent Self-Evolution extends classical RL in several ways. First, the state space includes not just the environment state but also the agent's own parameters, prompts, code, and memories---the agent can modify its own policy architecture, not just its weights. Second, the reward function may itself evolve (e.g., through evaluator co-training in Meta-Rewarding). Third, the action space is combinatorial and discrete (natural language prompts, code edits), making standard RL algorithms difficult to apply directly. RAGEN's StarPO framework represents one attempt to adapt RL to this setting by treating entire multi-turn trajectories as the unit of optimization.

\subsubsection{Continual Learning}

Continual learning addresses the challenge of learning new tasks without forgetting old ones. Agent Self-Evolution faces the same challenge when evolving across tasks: improvements for new task domains should not degrade performance on previously mastered tasks. The standard continual-learning toolbox---experience replay, elastic weight consolidation, progressive neural networks---has direct analogues in Agent Self-Evolution. DGM's archive serves as a form of experience replay; regression test suites play the role of consolidation constraints; the multi-layer state representation allows localized updates that minimize interference.

\subsubsection{AutoML and Neural Architecture Search}

AutoML and NAS provide the most direct precursors to Agent Self-Evolution. AutoML-GPT, AutoML-Agent, and related systems treat the ML pipeline as a searchable artifact, with LLMs replacing hand-crafted search heuristics. NAS systems like EvoPrompting and LLMatic use LLMs as architecture mutation operators. The key extension in Agent Self-Evolution is that the search target is not just a model architecture or a hyperparameter configuration but an \emph{entire agent system}---its prompts, tools, memories, code, and multi-agent topology.

\subsubsection{Program Synthesis and Self-Programming}

The connection to program synthesis is particularly relevant for systems like SICA, DGM, and AlphaEvolve, where the agent generates and modifies executable code. Classical program synthesis assumes a fixed specification and searches for a program that satisfies it. Agent Self-Evolution relaxes both assumptions: the specification may be ambiguous or evolving (the Specification-to-Execution Loop), and the program being synthesized may modify the synthesizer itself (the G\"{o}del machine dimension). This creates a form of recursive program synthesis where the boundary between ``program'' and ``program generator'' is fluid.
...
The fourth phase (2025--2026) is marked by growing attention to safety, memory governance, and production readiness. Misevolution analysis identifies failure modes specific to self-evolving systems. OEP demonstrates experience-poisoning attacks. Capability Erosion studies catastrophic forgetting in self-evolving agents. AriadneMem and Memory-R1 develop sophisticated memory management policies. This phase represents the maturation of the field from ``can we make agents self-improve?'' to ``how do we make self-improvement reliable, safe, and cost-effective?''

\subsubsection{Cross-Domain Convergence}

A striking feature of the field is the convergence of five previously separate research traditions:
\begin{enumerate}
\item \textbf{AutoML + LLM}: AutoML-GPT, AutoML-Agent, and related systems treat ML pipeline generation as a specification-to-execution problem, with LLMs replacing hand-crafted search heuristics. The ``evolved object'' is the ML pipeline; the generator is a planner or multi-agent team; the evaluator is dataset performance.
\item \textbf{NAS + LLM}: EvoPrompting, LLMatic, and NADER treat neural architecture design as an evolutionary search problem with LLMs as mutation operators. The ``evolved object'' is the network architecture; the generator is the LLM; the evaluator is accuracy/cost/robustness.
\item \textbf{LLM Self-Improvement}: Self-Refine, Reflexion, STaR, and SPIN focus on making individual LLMs better through self-feedback. The ``evolved object'' is the model output or weights; the generator and evaluator are the same or peer models; the memory is reflective text or fine-tuning data.
\item \textbf{Evolutionary Computation + LLM}: OPRO, FunSearch, ReEvo, and LLaMEA use LLMs as evolutionary optimizers. The ``evolved object'' is a program, prompt, or heuristic; the generator is the LLM; the evaluator is the objective function; the memory is the population archive.
\item \textbf{Agent Frameworks}: AutoGPT, MetaGPT, AutoGen, CrewAI, and related frameworks provide the orchestration layer for multi-agent systems. While most current frameworks lack persistent evolution mechanisms, they provide the infrastructure upon which evolution layers can be built.
\end{enumerate}
