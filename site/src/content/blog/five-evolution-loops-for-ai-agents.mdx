...
Source: `paper-drafts/ch2-taxonomy.tex`。第二章建立论文的核心分类框架：Five Evolution Loops。
...
- 哪些用户输入需要澄清，哪些可以直接执行；
- 哪些任务需要先读 raw，哪些先查 wiki；
- 哪些输出必须带证据链；
- 哪些操作必须先跑 git status 和引用搜索。

这也是为什么 Self Evolve 仓库强调 AGENTS、wiki、index 和验证命令：它们都是 specification-to-execution loop 的外部骨架。
...
```yaml
failure: tool call selected wrong file
condition: user asked for repo-wide analysis
fix: run rg before opening individual files
scope: coding/research tasks
expires: when repo index changes
```
...
Population loop 来自进化计算：保留多个候选分支，而不是只保留当前最高分版本。DGM、AlphaEvolve、MAP-Elites 这类系统都说明了一个原则：短期不是最强的变体，可能是长期 stepping stone。
