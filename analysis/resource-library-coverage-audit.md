...
| Work | Site route source files | 33 | `site/src/pages` 页面源码数。 | `rg --files site/src/pages` |
...
| Work | Wiki files | 60 | LLM Wiki 页面与搜索索引文件数，包含 coverage、topic-map 和 definition synthesis。 | `rg --files work/wiki -g '*.md' -g '*.json'` |
...
- [KNOWN] Public surface: Astro site、资料库覆盖页、主题地图页、定义主题页、博客/SEO、research pages、project reports、graph、paper page、survey page、rankings 和 Evolve-AGI Index page 已经存在。Source: `site/src/`, `site/public/reports/`
...
| 3 | SEO 主题簇已有 topic map 和 Rank 1 定义页，但还没拆成完整页面簇 | SEO 页面应来自真实证据簇，而不是泛泛关键词。 | 继续生成 loop 页、benchmark matrix、Agent-Swarm topology matrix 和 pain-point FAQ。 |
