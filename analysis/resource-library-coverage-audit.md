...
1. [KNOWN] 当前全仓库治理口径显示：639 个 raw GitHub captures、639 个 classified repositories、232 个 analyzed model-card reports、92 个 strict evolution repositories、199 个 broad evolution repositories、201 个 raw paper files、171 个 paper reviews 和 434 个 public project reports。Source: `docs/indexes/master-index.md`, `analysis/github-project-data-analysis.md`
...
3. 最需要对外解释的是计数口径：639 raw captures、232 analyzed model-card reports、434 public report files、232 site project records、801 project markdown files 分别代表不同层，不应该在 README 或论文里混成一个单一数字。
...
|---|---:|---:|---|---|
| Raw | Raw GitHub captures | 639 | 已进入 timestamp index 的 GitHub 原始采集记录。 | `docs/indexes/master-index.md` |
| Raw | Files under `raw-github/` | 640 | 磁盘文件数；可能包含 index/support 文件，不等于 capture 条目。 | `rg --files raw-github` |
...
| Processed | Classified GitHub repositories | 639 | 每个 raw GitHub capture 对应的分类行。 | `analysis/github-project-data-analysis.md` |
| Processed | Strict evolution repositories | 92 | `base_theme=evolution` 的严格进化主题仓库。 | `analysis/github-project-data-analysis.md` |
| Processed | Broad evolution repositories | 199 | 通过 self-improvement/evolution/reflection/search 等信号扩展的广义相关仓库。 | `analysis/github-project-data-analysis.md` |
| Processed | Analyzed model-card reports | 232 | 进入项目 model-card/public report 漏斗的分析对象。 | `docs/indexes/master-index.md` |
| Processed | Project markdown files | 801 | `projects/` 下全部 markdown；包含 index、archive、support、legacy 等，不等于公开展示项目数。 | `rg --files projects -g '*.md'` |
...
| Work | Site project records | 232 | `site/src/data/projects.ts` 当前可导入项目数组条目；是网站展示口径。 | `esbuild site/src/data/projects.ts` then inspect `projects.length` |
...
| Work | Wiki files | 61 | LLM Wiki 页面与搜索索引文件数，包含 coverage、topic-map、definition 和 five-loop synthesis。 | `rg --files work/wiki -g '*.md' -g '*.json'` |
| Results | Public project reports | 434 | 静态站点可引用/下载的公开项目报告文件。 | `docs/indexes/master-index.md` |

## Same Number, Different Meaning / 口径边界

| Number | Do Not Say | Say This Instead |
|---:|---|---|
| 639 | “我们分析了 639 个项目并都有 model card。” | “我们有 639 个 raw GitHub captures，并且 639 个进入 classified repository 表。” |
| 232 | “网站只有 232 个项目。” | “当前治理索引显示 232 个进入 analyzed model-card reports 漏斗。” |
| 434 | “我们有 434 个不同项目。” | “`site/public/reports/projects/` 有 434 个 public project report 文件。” |
| 232 | “项目库只有 232 个。” | “`site/src/data/projects.ts` 当前网站展示数组有 232 个项目条目。” |
| 801 | “我们深度分析了 801 个项目。” | “`projects/` 下有 801 个 markdown 文件，包含项目卡、索引、支持材料和历史/兼容内容。” |
...
| 4 | 项目展示数组小于分析/report 库 | 网站 `/projects/` 当前是展示口径，不能代表全部资料库。 | 增加 all reports / coverage entry，避免读者误会只有 232 个项目。 |
