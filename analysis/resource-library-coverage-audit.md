...
1. [KNOWN] 当前全仓库治理口径显示：646 个 raw GitHub captures、646 个 classified repositories、239 个 analyzed model-card reports、93 个 strict evolution repositories、200 个 broad evolution repositories、201 个 raw paper files、171 个 paper reviews 和 441 个 public project reports。Source: `docs/indexes/master-index.md`, `analysis/github-project-data-analysis.md`
...
3. 最需要对外解释的是计数口径：646 raw captures、239 analyzed model-card reports、441 public report files、239 site project records、808 project markdown files 分别代表不同层，不应该在 README 或论文里混成一个单一数字。
...
|---|---:|---:|---|---|
| Raw | Raw GitHub captures | 646 | 已进入 timestamp index 的 GitHub 原始采集记录。 | `docs/indexes/master-index.md` |
| Raw | Files under `raw-github/` | 647 | 磁盘文件数；可能包含 index/support 文件，不等于 capture 条目。 | `rg --files raw-github` |
...
| Processed | Classified GitHub repositories | 646 | 每个 raw GitHub capture 对应的分类行。 | `analysis/github-project-data-analysis.md` |
| Processed | Strict evolution repositories | 93 | `base_theme=evolution` 的严格进化主题仓库。 | `analysis/github-project-data-analysis.md` |
| Processed | Broad evolution repositories | 200 | 通过 self-improvement/evolution/reflection/search 等信号扩展的广义相关仓库。 | `analysis/github-project-data-analysis.md` |
| Processed | Analyzed model-card reports | 239 | 进入项目 model-card/public report 漏斗的分析对象。 | `docs/indexes/master-index.md` |
| Processed | Project markdown files | 808 | `projects/` 下全部 markdown；包含 index、archive、support、legacy 等，不等于公开展示项目数。 | `rg --files projects -g '*.md'` |
...
| Work | Site project records | 239 | `site/src/data/projects.ts` 当前可导入项目数组条目；是网站展示口径。 | `esbuild site/src/data/projects.ts` then inspect `projects.length` |
| Work | Site route source files | 34 | `site/src/pages` 页面源码数；新增 code-evolution benchmark 主题页，并移除不可达的 `/visualizations` 重复路由。 | `rg --files site/src/pages` |
...
| Work | Wiki files | 63 | LLM Wiki 页面与搜索索引文件数，包含 coverage、topic-map、definition、five-loop、code-benchmark 和 value-screening synthesis。 | `rg --files work/wiki -g '*.md' -g '*.json'` |
| Results | Public project reports | 441 | 静态站点可引用/下载的公开项目报告文件。 | `docs/indexes/master-index.md` |

## Same Number, Different Meaning / 口径边界

| Number | Do Not Say | Say This Instead |
|---:|---|---|
| 646 | “我们分析了 646 个项目并都有 model card。” | “我们有 646 个 raw GitHub captures，并且 646 个进入 classified repository 表。” |
| 239 | “网站只有 239 个项目。” | “当前治理索引显示 239 个进入 analyzed model-card reports 漏斗。” |
| 441 | “我们有 441 个不同项目。” | “`site/public/reports/projects/` 有 441 个 public project report 文件。” |
| 239 | “项目库只有 239 个。” | “`site/src/data/projects.ts` 当前网站展示数组有 239 个项目条目。” |
| 808 | “我们深度分析了 808 个项目。” | “`projects/` 下有 808 个 markdown 文件，包含项目卡、索引、支持材料和历史/兼容内容。” |
...
- [KNOWN] Public surface: Astro site、资料库覆盖页、主题地图页、定义主题页、五类回路主题页、代码自改进 benchmark 页、博客/SEO、research pages、project reports、graph、paper page、survey page、rankings 和 Evolve-AGI Index page 已经存在。Source: `site/src/`, `site/public/reports/`
...
| 3 | SEO 主题簇已有 topic map、Rank 1 定义页、Rank 2 五类回路页和 Rank 3 code-evolution benchmark 页，但还没拆成完整页面簇 | SEO 页面应来自真实证据簇，而不是泛泛关键词。 | 继续生成单 loop 页、代表系统页、Agent-Swarm topology matrix 和 pain-point FAQ。 |
| 4 | 项目展示数组小于分析/report 库 | 网站 `/projects/` 当前是展示口径，不能代表全部资料库。 | 增加 all reports / coverage entry，避免读者误会只有 239 个项目。 |
...
- [KNOWN] Site project display count was measured by bundling `site/src/data/projects.ts` and inspecting `projects.length`.
...
- [KNOWN] Public definition page now lives at `site/src/pages/topics/self-evolving-ai-agents/index.astro`.
- [KNOWN] Public code-evolution benchmark page now lives at `site/src/pages/topics/code-evolution-benchmark/index.astro`.
- [KNOWN] Reader-facing vs internal workflow boundaries now live at `docs/ops/audience-boundary-workflow.md`.
