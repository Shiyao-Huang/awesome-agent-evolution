...
const runAt = '2026-05-25T23:35:00+08:00';
...
const repos = [
  {
    ...
    tags: ['agent memory', 'research index', 'taxonomy'],
    ...
    evidence: 'web-observed GitHub page showed MIT license, 10 stars, 3 forks, 16 commits, and an MCP-centric memory server/tool list covering Basic Memory, OpenMemory MCP, graph memory servers and vector-backed memory tools; counts were not API-verified in this shell-blocked run.',
    excerpts: [
      'Public result describes the repository as a curated list of memory systems, tools, patterns and research.',
      'The repository is useful as a taxonomy/checklist source rather than a runnable memory runtime.',
      'Public GitHub page showed MIT license, 10 stars and 3 forks; these are web-observed counts, not GitHub API freshness.'
    ]
  },
  {
    name: 'Memoir',
    ...
    rawFile: 'raw-github/zhangfengcdt_memoir.md',
    ...
    evidence: 'web-observed GitHub page showed 155 commits, Apache-2.0 license, 549 stars, 35 forks, Python/Shell/TypeScript stack, v0.2.0 release on 2026-05-11, Codex/Claude plugin install paths, and README language about Git-like branching/commit/merge/rollback for AI memory; no benchmark was rerun.',
    excerpts: [
      'Public result calls Memoir "Git for Agent Auto-Memory".',
      'README emphasizes semantic paths, branch-aware memory, rollback, memory aggregation, and multiple search engines.',
      'The page includes Codex and Claude Code plugin setup paths, so the project is relevant to agent memory infrastructure.'
    ]
  },
  {
    ...
    evidence: 'web-observed GitHub page showed 108 commits, 273 stars, 15 forks and a graph-based agent memory survey/resource list covering taxonomy, memory extraction/storage/retrieval/evolution, benchmarks and projects.',
    excerpts: [
      'Public result presents the project as a graph-based agent memory paper/resource list.',
      'The relevance is graph memory taxonomy, not a directly executed agent harness.',
      'Counts and recency remain web-observed only because shell GitHub API access failed.'
    ]
  },
  {
    name: 'ATANT',
    ...
    rawFile: 'raw-github/kenotic-labs_atant.md',
    ...
    evidence: 'web-observed GitHub page showed 15 commits, 3 stars, 0 forks, Python language, continuity benchmark topics, 250 narrative stories, 1,835 verification questions, 10 checkpoints, no LLM in the evaluation loop, arXiv links and Hugging Face corpus link.',
    excerpts: [
      'Public result frames ATANT as an evaluation framework for AI continuity.',
      'The project expands the benchmark axis from task score to continuity and narrative-state behavior.',
      'No local benchmark execution or API-verified metadata was available in this run.'
    ]
  },
  {
    name: 'Gitagent',
    ...
    rawFile: 'raw-github/open-gitagent_gitagent.md',
    ...
    evidence: 'web-observed GitHub page showed MIT license, 404 stars, 82 forks, 123 commits, and described a universal git-native AI agent framework with agent.yaml, SOUL.md, RULES.md, git-committed memory, declarative tools, composable skills, hooks, plugins, compliance audit logs and OpenTelemetry instrumentation.',
    excerpts: [
      'README says the agent lives inside a git repo with identity, rules, memory, tools and skills as version-controlled files.',
      'Quickstart auto-scaffolds agent.yaml, SOUL.md and memory, and architecture docs show tools, skills, workflows, agents, plugins and hooks.',
      'The project is relevant as a harness/runtime pattern for auditable self-evolving agent state, not as a benchmark result.'
    ]
  },
  {
    ...
    rawFile: 'raw-github/google-gemini_gemini-cli.md',
    ...
    base_theme: 'memory',
    stack: 'TypeScript/CLI/Auto-memory/Skills',
    language: 'TypeScript',
    license: 'Apache-2.0',
    stars: 105000,
    forks: '13900',
    ...
    tags: ['agent memory', 'skills', 'cli', 'workflow automation'],
    description: 'Gemini CLI is a major open-source terminal agent; its Auto Memory docs add a mainstream implementation path for mining past sessions into reviewable durable memory and reusable Agent Skills.',
    evidence: 'web-observed GitHub page showed Apache-2.0 license, 105k stars, 13.9k forks, 6,224 commits, memory-tests/evals directories, v0.43.0 latest release on 2026-05-22, and README claims for an open-source terminal AI agent; Auto Memory docs say it mines past sessions for durable memory and reusable Agent Skills, drafts patches or SKILL.md files, and never applies them without approval.',
    excerpts: [
      'Repository README positions Gemini CLI as an open-source terminal AI agent with built-in tools, MCP support, checkpointing and custom context files.',
      'Auto Memory documentation says it scans past sessions for durable facts, workflow constraints and procedural patterns, then drafts memory patches or SKILL.md files.',
      'The mechanism is explicitly review-gated, making it a high-value mainstream example for memory and skill evolution governance.'
    ]
  }
];
...
function updateProjectsData() {
  ...
  source = addToGroup(source, '个人 Agent 产品与真实评测', ['zhangfengcdt/memoir', 'Kenotic-Labs/ATANT', 'google-gemini/gemini-cli']);
  source = addToGroup(source, 'Harness 进化与方法索引', ['open-gitagent/gitagent', 'google-gemini/gemini-cli']);
  write(file, source);
}
...
function broadEvolution(row) {
  return [row.repo, row.description, row.base_theme, row.function_tag, row.evidence].join(' ').toLowerCase().match(/evol|self-improv|self improv|self-evolv|reflexion|self-refine|darwin|godel|gödel|gepa|funsearch|alph[a]?evolve|recursive/);
}

function benchmarkRelated(row) {
  return [row.final_category, row.function_tag, row.description, row.evidence].join(' ').toLowerCase().match(/benchmark|eval|评测/);
}
...
function updateGraphAndNotes() {
  ...
  if (workNode) workNode.paths = [...new Set([...(workNode.paths || []), 'work/research/hourly-public-metadata-update-2026-05-25-1648.md'])];
  graph.current_run = {
    run_at: runAt,
    added_repositories: repos.map((repo) => repo.repo),
    evidence_quality: 'web_observed_public_pages_and_search; shell GitHub API DNS failed and gh token invalid',
    graph_truth_source: '.mermaid/current/graph.json',
    gitnexus_status: 'available; npx gitnexus status reported a stale index before this iteration and recommends re-running gitnexus analyze'
  };
