...
const runAt = '2026-06-15T08:24:11+08:00';
const runDate = '2026-06-15';
const runDisplay = '2026-06-15 08:24 +0800';
const runSlug = '2026-06-15-0824';
const timestampSource = 'gh_api_graphql_authenticated_2026_06_15';
const previousDataPath = 'work/research/hourly-public-metadata-update-2026-06-14-1422-data.json';
const gitNexusBlocker =
  'LadybugDB unavailable for awesome-evolution-workspace-cleanup. Another process may be rebuilding the index. Retry later. (Runtime exception: Trying to read a database file with a different version. Database file version: 41, Current build storage version: 40)';
...
const repoConfigs = [
  {
    ...
    project: {
      ...
      threeSentences: [
        ...
        'The 2026-06-15 packet shows the anchor is still moving publicly, so the evidence chain needs to stay synced through raw, processed, and public-site layers.'
      ],
      teaching:
  ...
  },
  {
    ...
    raw: {
      ...
      bullets: [
        ...
        'This 2026-06-15 packet keeps Hermes on the freshest available authenticated metadata before any claim about product momentum or runtime maturity.'
      ]
    },
    project: {
      ...
      threeSentences: [
        ...
        'The 2026-06-15 authenticated packet shows the runtime continues to move at public GitHub scale, so the metadata surface has to stay fresh.'
      ],
      teaching:
  ...
  },
  {
    ...
    project: {
      ...
      threeSentences: [
        ...
        'This 2026-06-15 refresh keeps the outer-loop reference on the newest authenticated GitHub packet.'
      ],
      teaching:
  ...
  },
  {
    ...
    raw: {
      ...
      bullets: [
        ...
        'This 2026-06-15 refresh ensures the memory-substrate anchor is carried by current authenticated counts rather than stale star badges or local summaries.'
      ]
    },
    project: {
      ...
      threeSentences: [
        ...
        'This 2026-06-15 packet keeps the public memory evidence synchronized with the latest available GitHub metadata.'
      ],
      teaching:
  ...
  },
  {
    ...
    raw: {
      ...
      bullets: [
        ...
        'This 2026-06-15 packet keeps the evaluator substrate synchronized with the latest authenticated metadata even when the benchmark story itself is stable.'
      ]
    },
    project: {
      ...
      threeSentences: [
        ...
        'This 2026-06-15 packet confirms the benchmark anchor on a fresh authenticated snapshot.'
      ],
      teaching:
  ...
  },
  {
    ...
    raw: {
      ...
      bullets: [
        ...
        'This 2026-06-15 packet keeps the taxonomy-comparator surface on a current authenticated baseline.'
      ]
    },
    project: {
      ...
      threeSentences: [
        ...
        'This 2026-06-15 refresh keeps the comparator surface synchronized with the newest public metadata.'
      ],
      teaching:
  ...
  },
  {
    ...
    raw: {
      ...
      bullets: [
        ...
        'This 2026-06-15 packet keeps the swarm concept anchor refreshed even though it remains outside the public site registry.'
      ]
    },
...
function summarizeStatusZh(prev, next) {
  ...
  return changes.length ? changes.join('；') + '。' : '与 2026-06-14 packet 相比无公开数字变化。';
}

function summarizeStatusEn(prev, next) {
  const changes = summarizeChanges(prev, next);
  return changes.length ? `${changes.join('; ')}.` : 'No public metadata delta relative to the 2026-06-14 packet.';
}
...
- Relative to the previous 2026-06-14 packet: ${summarizeStatusEn(prev, meta)}
...
The raw capture now reflects an authenticated GitHub API snapshot: ${formatNumber(meta.stargazerCount)} stars, ${formatNumber(meta.forkCount)} forks, ${formatNumber(meta.defaultBranchRef?.target?.history?.totalCount ?? 0)} commits, ${formatNumber(meta.issues?.totalCount ?? 0)} open issues, and ${formatNumber(meta.pullRequests?.totalCount ?? 0)} open pull requests observed on ${runDate}. Relative to the previous 2026-06-14 packet: ${summarizeStatusEn(prev, meta)} ${config.project.limits}
`;
}
...
function replaceSection(filePath, startHeading, endHeading, replacement) {
  ...
  if (start === -1 || end === -1) {
    throw new Error(`Failed to locate section ${startHeadings.join(' | ')} in ${filePath}`);
  }
...
function buildReadmeSectionZh(previousByRepo, metadataByRepo, includeEvidenceColumn) {
  ...
  const title = includeEvidenceColumn ? '## 近期证据更新（2026-06-15）' : '## 本轮 GitHub Metadata 修复包（2026-06-15）';
  const intro =
    `本轮是新的 authenticated GitHub API packet，而不是沿用 2026-06-14 的上一轮快照。相对 \`2026-06-14 14:22 +0800\`，8 个锚点里有 ${deltaCount} 个出现新的公开 metadata 变化；唯一保持不变的是 Insight Swarm。`;
...
function buildReadmeSectionEn(previousByRepo, metadataByRepo) {
  const deltaCount = repoConfigs.filter((config) => summarizeChanges(previousByRepo.get(config.repo), metadataByRepo.get(config.repo)).length).length;
  const title = '## Recent Evidence Updates (2026-06-15)';
  const intro =
    `This is a new authenticated GitHub API packet rather than a reuse of the 2026-06-14 snapshot. Relative to \`2026-06-14 14:22 +0800\`, ${deltaCount} of the 8 tracked anchors now show fresh public metadata movement; only Insight Swarm remained unchanged.`;
  const rows = repoConfigs
    .map((config) => {
  ...
  const graph = {
    ...
    current_run: {
      ...
      gitnexus: {
        ...
        note: 'node .gitnexus/run.cjs status works; query requires --repo and then fails while loading LadybugDB.',
        blocker: gitNexusBlocker
      }
    }
  };
...
- Relative to the 2026-06-14 14:22 packet, ${deltaCount} of the 8 anchors moved on public metadata; only Insight Swarm stayed unchanged.
...
Fetched a new authenticated GitHub metadata packet on ${runDate}, found fresh public deltas on 6 of the 8 tracked anchors, and propagated that newer state through raw, processed, public-site, and Mermaid surfaces.
...
- Fresh public deltas versus the 2026-06-14 14:22 packet: ${deltaConfigs.map((config) => `\`${config.repo}\``).join(', ')}.
- No public-count delta versus the 2026-06-14 14:22 packet: ${stableConfigs.map((config) => `\`${config.repo}\``).join(', ')}.
...
- Updated the Mermaid truth source and rendered views so the current object graph reflects the ${runDate} packet and the live GitNexus blocker.
...
function main() {
  ...
  replaceSection('README.md', ['## 近期证据更新（2026-06-14）', '## 近期证据更新（2026-06-15）'], '## 核心洞察', buildReadmeSectionZh(previousByRepo, metadataByRepo, true));
  replaceSection('README-ZH.md', ['## 本轮 GitHub Metadata 修复包（2026-06-14）', '## 本轮 GitHub Metadata 修复包（2026-06-15）'], '## 核心洞察', buildReadmeSectionZh(previousByRepo, metadataByRepo, false));
  replaceSection('README-EN.md', ['## Recent Evidence Updates (2026-06-14)', '## Recent Evidence Updates (2026-06-15)'], '## Core Insight', buildReadmeSectionEn(previousByRepo, metadataByRepo));
