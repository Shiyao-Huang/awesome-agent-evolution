...
Options:
  --provider value|hash|openai|gbrain Embedding provider. Default: value.
...
const parseArgs = (argv) => {
  ...
  if (!['value', 'hash', 'openai', 'gbrain'].includes(args.provider)) throw new Error(`Invalid --provider: ${args.provider}`);
...
const valueEmbedding = (row) => normalizeVector(Object.keys(row.values).sort().map((key) => Number(row.values[key] || 0)));
...
const tsne3 = (vectors, iterations = 360, learningRate = 80) => {
  const n = vectors.length;
  if (n <= 3) return pca3(vectors);
  const distances = pairwiseDistances(vectors);
  const nonzero = distances.flat().filter((value) => value > 0).sort((a, b) => a - b);
  const sigma = nonzero[Math.floor(nonzero.length * 0.2)] || nonzero[Math.floor(nonzero.length / 2)] || 1;
  const p = Array.from({ length: n }, () => Array.from({ length: n }, () => 0));
  let pSum = 0;
  for (let i = 0; i < n; i += 1) {
    for (let j = i + 1; j < n; j += 1) {
      const value = Math.exp(-(distances[i][j] ** 2) / (2 * sigma ** 2));
      p[i][j] = value;
      p[j][i] = value;
      pSum += value * 2;
    }
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < n; j += 1) p[i][j] = p[i][j] / (pSum || 1);
  }

  let y = vectors.map((_, index) => [
    Math.sin(index * 12.9898) * 0.01,
    Math.cos(index * 78.233) * 0.01,
    Math.sin(index * 37.719) * 0.01
  ]);
  let gains = y.map(() => [0, 0, 0]);
  for (let iter = 0; iter < iterations; iter += 1) {
    ...
    const exaggeration = iter < 90 ? 4 : 1;
    for (let i = 0; i < n; i += 1) {
      for (let j = 0; j < n; j += 1) {
        if (i === j) continue;
    ...
    y = y.map((point, i) => point.map((value, dim) => {
      gains[i][dim] = momentum * gains[i][dim] - learningRate * grads[i][dim];
      return clamp(value + gains[i][dim], -100, 100);
    }));
  }
  return y;
};
...
const main = async () => {
  ...
  const points = normalizePoints(args.projection === 'pca' ? pca3(vectors) : tsne3(vectors));
