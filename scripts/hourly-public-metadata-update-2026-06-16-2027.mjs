...
const runAt = '2026-06-16T20:27:47+08:00';
const runDate = '2026-06-16';
const runDisplay = '2026-06-16 20:27 +0800';
const runSlug = '2026-06-16-2027';
const timestampSource = 'gh_api_graphql_authenticated_2026_06_16_2027';
const previousPacketSlug = '2026-06-16-1428';
const previousPacketLabel = '2026-06-16 14:28 +0800';
...
本轮是新的 hourly public metadata 修复包，不再沿用 \`${previousPacketLabel}\` 的上一个 authenticated packet 作为唯一前台口径。抓取链路本身是抖动的而不是完全不可用，因此本轮按“先重试 live GitHub API，失败时明确回退到上一个 authenticated packet”的规则更新，避免伪造 freshness。
...
This is a fresh hourly public metadata repair packet rather than another reuse of the previous authenticated packet from \`${previousPacketLabel}\`. The GitHub lane in this workspace is flaky instead of fully unavailable, so this run follows a strict rule: retry live GitHub API fetches first, and only fall back to the previous authenticated packet when the live request fails, with the limit stated explicitly.
...
Repaired the hourly public metadata packet by rechecking the tracked GitHub anchors at ${runDisplay}, then propagated the fresher or explicitly-fallback state through raw captures, classification, README, Mermaid, site reports, and downstream index surfaces.
