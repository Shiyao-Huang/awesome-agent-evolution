...
const runAt = '2026-05-26T07:45:34+08:00';
const runDate = '2026-05-26';
const runNote = 'work/research/hourly-public-metadata-update-2026-05-26-0745.md';
...
const repos = [
  {
    ...
    rawFile: 'raw-github/agiprolabs_claude-trading-skills.md',
    ...
    base_theme: 'skill',
    stack: 'Python/Markdown/Agent Skills',
    language: 'Python',
    license: 'MIT',
    stars: 31,
    forks: 14,
    ...
    tags: ['agent skills', 'domain workflow', 'trading', 'Codex', 'Claude Code'],
    ...
    evidence: 'web-observed GitHub page showed 31 stars, 14 forks, 4 commits, MIT license, Python primary language, and README language about 62 trading/DeFi/quantitative-finance Agent Skills across 16 categories with Claude Code plugin and manual install paths for Codex, Cursor and Gemini CLI.',
    excerpts: [
      'The public README describes 62 trading, DeFi and quantitative finance Agent Skills.',
      'Visible workflows cover market data, on-chain research, backtesting, portfolio/risk, DeFi analysis, execution, tax, accounting and compliance.',
      'The install path includes Claude Code plugin usage plus manual copy targets for Claude Code, Cursor, Codex and Gemini CLI.',
      'The repository is a domain skill asset layer, not an autonomous self-evolution algorithm.'
    ]
  },
  {
    ...
    rawFile: 'raw-github/dceoy_speckit-agent-skills.md',
    ...
    tags: ['agent skills', 'spec-driven development', 'workflow orchestration', 'Codex', 'Gemini CLI'],
    ...
    evidence: 'web-observed GitHub page showed 88 stars, 21 forks, 92 commits, AGPL-3.0 license, Shell primary language, v0.1.2 release on 2026-03-14, and README sections for shared speckit-* skills plus runtime-specific Claude Code, Codex, GitHub Copilot CLI and Gemini CLI access.',
    excerpts: [
      'The public README describes reusable skills and templates for multiple agent runtimes.',
      'Visible structure includes shared skills, Claude commands, Codex prompts, GitHub Copilot agents/prompts, Gemini commands and Spec Kit helper scripts.',
      'The workflow sequence is constitution, specify, optional clarify, plan, optional analyze, tasks and implement.',
      'The project is relevant as a process-governance skill layer for agent work, not as a benchmark result.'
    ]
  }
];
...
function modelCard(repo) {
  return `# ${repo.name} Model Card\n\n| Field | Value |\n|---|---|\n| Repository | [${repo.repo}](${repo.url}) |\n| Category | ${repo.category} |\n| Stars / forks snapshot | ${repo.stars} / ${repo.forks} |\n| Language | ${repo.language} |\n| License | ${repo.license} |\n| Raw capture | ${repo.rawFile} |\n| Updated by | hourly public metadata update, 2026-05-26 07:45 +0800 |\n\n## 1. Role in Self Evolve\n\n${repo.description} It matters to self-evolving agents because durable improvement often appears first as reusable procedures, workflow discipline and domain-specific skill assets rather than direct model-weight updates.\n\n## 2. Working Principle\n\n${repo.pattern}\n\n## 3. Evidence Path\n\n${repo.evidence} Shell GitHub API access remained blocked by DNS and local \`gh\` auth was invalid, so this card treats the snapshot as web-observed rather than API-verified.\n\n## 4. Teaching Use\n\nUse this card to explain ${repo.category}: the project helps show how skill libraries turn repeated expert behavior into installable agent assets that can be versioned, reviewed and reused across agents.\n\n## 5. Limits\n\nThe repository was not cloned in this iteration; no benchmark, plugin install, workflow run, trading analysis, Spec Kit task, or agent loop was executed. Counts and claims are visible public-page/search signals unless independently revalidated later.\n`;
}
