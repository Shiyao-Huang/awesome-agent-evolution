...
const runAt = '2026-06-16T02:26:37+08:00';
const runDate = '2026-06-16';
const runDisplay = '2026-06-16 02:26 +0800';
const runSlug = '2026-06-16-0226';
const timestampSource = 'gh_api_graphql_authenticated_2026_06_16';
const previousDataPath = 'work/research/hourly-public-metadata-update-2026-06-15-2024-data.json';
...
function summarizeStatusZh(prev, next) {
  ...
  return changes.length ? `${changes.join('；')}。` : '与 2026-06-15 20:24 packet 相比无公开数字变化。';
}

function summarizeStatusEn(prev, next) {
  const changes = summarizeChanges(prev, next);
  return changes.length ? `${changes.join('; ')}.` : 'No public metadata delta relative to the 2026-06-15 20:24 packet.';
}
...
- Relative to the previous 2026-06-15 20:24 packet: ${summarizeStatusEn(prev, meta)}
...
The raw capture now reflects an authenticated GitHub API snapshot: ${formatNumber(meta.stargazerCount)} stars, ${formatNumber(meta.forkCount)} forks, ${formatNumber(meta.defaultBranchRef?.target?.history?.totalCount ?? 0)} commits, ${formatNumber(meta.issues?.totalCount ?? 0)} open issues, and ${formatNumber(meta.pullRequests?.totalCount ?? 0)} open pull requests observed on ${runDate}. Relative to the previous 2026-06-15 20:24 packet: ${summarizeStatusEn(prev, meta)} ${config.project.limits}
`;
}
...
function buildReadmeSectionZh(previousByRepo, metadataByRepo, includeEvidenceColumn) {
  ...
  const intro =
    `本轮是新的 authenticated GitHub API packet，而不是沿用 \`2026-06-15 20:24 +0800\` 的晚间快照。相对该 packet，${repoConfigs.length} 个锚点里有 ${deltaCount} 个出现新的公开 metadata 变化；保持不变的条目是 ${stableNames.join('、') || '无'}。`;
...
function buildReadmeSectionEn(previousByRepo, metadataByRepo) {
  ...
  const intro =
    `This is a new authenticated GitHub API packet rather than a reuse of the evening \`2026-06-15 20:24 +0800\` snapshot. Relative to that packet, ${deltaCount} of the ${repoConfigs.length} tracked anchors now show fresh public metadata movement; the stable entries are ${stableNames.join(', ') || 'none'}.`;
  const rows = repoConfigs
    .map((config) => {
  ...
  const graph = {
    ...
    current_run: {
      ...
      gitnexus: {
        ...
        note: 'npx gitnexus status works and the direct query lane still fails on LadybugDB version mismatch.',
...
- Relative to the 2026-06-15 20:24 packet, ${deltaCount} of the ${repoConfigs.length} anchors moved on public metadata.
...
- Fresh public deltas versus the 2026-06-15 20:24 packet: ${deltaConfigs.map((config) => `\`${config.repo}\``).join(', ')}.
- No public-count delta versus the 2026-06-15 20:24 packet: ${stableConfigs.map((config) => `\`${config.repo}\``).join(', ')}.
