...
## GitNexus 知识图谱分析
- **分析命令**：`gitnexus analyze repos/aiwaves_cn__agents --index-only --skip-git --name ae-aiwaves_cn__agents`。
- **图谱规模**：14149 nodes / 15626 edges / 56 clusters / 93 flows。
- **查询语句**：`self evolving agent data feedback action memory execution`。
- **相关执行流程**：
  - __init__ → Load_from_json
  - __init__ → Load_from_json
  - Execute → Encode_memory
  - Formulate_prompt_for_node_backward → Get_role_chat
- **关键符号/文件**：
  - `agent_team.py`（`src/agents/agents/agent_team.py`）
  - `AgentTeam`（`src/agents/agents/agent_team.py`）
  - `agent.py`（`src/agents/agents/agent.py`）
  - `Agent`（`src/agents/agents/agent.py`）
  - `prompt_formatter.py`（`src/agents/optimization/prompt_formatter.py`）
  - `environment.py`（`src/agents/agents/environment.py`）
- **符号上下文**：
  - 符号 `agent_team.py` 位于 `src/agents/agents/agent_team.py` 第 ?–? 行
  - 入边/被依赖关系：imports: 3
