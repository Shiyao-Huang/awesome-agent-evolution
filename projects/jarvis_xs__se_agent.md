...
## GitNexus 知识图谱分析
- **分析命令**：`gitnexus analyze repos/jarvis_xs__se_agent --index-only --skip-git --name ae-jarvis_xs__se_agent`。
- **图谱规模**：4510 nodes / 8103 edges / 225 clusters / 192 flows。
- **查询语句**：`revision recombination refinement self evolution code agent`。
- **相关执行流程**：
  - Process_file → Is_bad
  - Process_file → Filter_non_empty
  - Process_file → Filter_unique
  - Process_file → Filter_length
- **关键符号/文件**：
  - `revise_history_before_query`（`sweagent/agent/hooks/abstract.py`）
  - `revise_history_before_query`（`sweagent/agent/hooks/abstract.py`）
  - `abstract.py`（`sweagent/agent/hooks/abstract.py`）
  - `README.md`（`README.md`）
  - `RunReplay`（`sweagent/run/run_replay.py`）
  - `__init__`（`sweagent/run/run_replay.py`）
- **符号上下文**：
  - 符号 `revise_history_before_query` 位于 `sweagent/agent/hooks/abstract.py` 第 33–44 行
  - 入边/被依赖关系：calls: 1，has_method: 1
