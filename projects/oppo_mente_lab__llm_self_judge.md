...
## GitNexus 知识图谱分析
- **分析命令**：`gitnexus analyze repos/oppo_mente_lab__llm_self_judge --index-only --skip-git --name ae-oppo_mente_lab__llm_self_judge`。
- **图谱规模**：10236 nodes / 18110 edges / 354 clusters / 300 flows。
- **查询语句**：`self judge evaluation reasoning inference train`。
- **相关执行流程**：
  - Compute_rm_score → _clamp_01
  - Generate_sequences → From_str
- **关键符号/文件**：
  - `BaseEngine`（`verl/workers/engine/base.py`）
  - `initialize`（`verl/workers/engine/base.py`）
  - `base.py`（`verl/workers/engine/base.py`）
  - `verifier.py`（`verl/workers/reward_model/verifier/verifier.py`）
  - `qwen2_5_extract_judge`（`verl/workers/reward_model/verifier/verifier.py`）
  - `verifier.py`（`verl/workers/reward_manager/verifier.py`）
- **符号上下文**：
  - 符号 `BaseEngine` 位于 `verl/workers/engine/base.py` 第 25–189 行
  - 入边/被依赖关系：extends: 2，imports: 4
  - 出边/调用或包含关系：has_method: 17
