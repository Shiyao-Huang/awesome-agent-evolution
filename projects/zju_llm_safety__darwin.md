...
## GitNexus 知识图谱分析
- **分析命令**：`gitnexus analyze repos/zju_llm_safety__darwin --index-only --skip-git --name ae-zju_llm_safety__darwin`。
- **图谱规模**：1053 nodes / 1895 edges / 38 clusters / 68 flows。
- **查询语句**：`strategy evolution genetic reflection jailbreak evaluator`。
- **相关执行流程**：
  - Validate → Build_legacy_judge_messages
  - Validate → Build_markov_policy_judge_messages
  - Validate → Parse_legacy_pair_score
  - Validate → Parse_markov_policy_score
- **关键符号/文件**：
  - `main.py`（`main.py`）
  - `main`（`main.py`）
  - `genetic_evolution.py`（`evolution/genetic_evolution.py`）
  - `__init__`（`evolution/genetic_evolution.py`）
  - `GeneticEvolution`（`evolution/genetic_evolution.py`）
  - `reflective_evolution.py`（`evolution/reflective_evolution.py`）
- **符号上下文**：
  - 符号 `main.py` 位于 `main.py` 第 ?–? 行
  - 出边/调用或包含关系：calls: 1，imports: 8
